...
class _GlobalCallUIState extends State<GlobalCallUI> {
  bool _isMinimized = false; // WhatsApp-style: fullscreen by default
...
const _kWaBg = Color(0xFF0B141A);
const _kWaGreen = Color(0xFF25D366);
const _kWaRed = Color(0xFFF15C6D);
const _kWaMuted = Color(0xFF8696A0);
const _kWaBar = Color(0xFF1F2C34);

const _kBg = BoxDecoration(color: _kWaBg);
          ...
          child: Column(
            children: [
              ...
              Text(
                ...
                style: const TextStyle(color: _kWaMuted, fontSize: 15),
              ),
              const Spacer(flex: 3),
              Padding(
...
class _CallingPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final name = callee?.fullName ?? '';
    final isVideo = cp.callType == 'video';
    final showLocalPreview =
        isVideo && cp.engine != null && cp.localUid != null && cp.localVideoEnabled;
              ...
              if (showLocalPreview)
                AgoraVideoView(
                  controller: VideoViewController(
                    rtcEngine: cp.engine!,
                    canvas: VideoCanvas(uid: cp.localUid ?? cp.agoraUid ?? 0),
                  ),
                ),
              Column(
                children: [
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    _ToggleBtn(
                      ...
                      toggled: !cp.speakerEnabled,
                      icon: cp.speakerEnabled
                          ? Icons.volume_up
                          : Icons.volume_off,
                      label: cp.speakerEnabled ? 'Speaker' : 'Speaker off',
                      size: isMin ? 48 : 52,
                    ),
                  ],
                ),
...
class _VideoCallPanelState extends State<_VideoCallPanel> {
  ...
  String get _channelId =>
      widget.cp.activeChannel ??
      (widget.cp.activeConsultId != null
          ? 'consult-${widget.cp.activeConsultId}'
          : '${widget.cp.userId}-${widget.cp.currentCallee?.id ?? widget.cp.incomingCall?.fromId ?? ''}');
