...
class _MessageScreenState extends State<MessageScreen> with WidgetsBindingObserver {
  ...
  Future<void> _fetchConversations({bool silent = false}) async {
    ...
    try {
      ...
      final convos = data.map((chat) {
        ...
        final other = _findOtherParticipant(participants, currentId);
        final lastMsg = chat['lastMessage'];
        final timeSource = chat['lastActivity'] ??
            chat['updatedAt'] ??
            (lastMsg is Map ? lastMsg['createdAt'] : null);

        return ConversationModel(
          id: chat['_id']?.toString() ?? '',
          name: other['fullName']?.toString() ?? 'Unknown User',
          avatar: other['profile_image']?.toString(),
          lastMessage: _getLastMessageText(lastMsg),
  ...
  Map<String, dynamic> _findOtherParticipant(
    ...
    if (currentId != null) {
      for (final p in participants) {
        if (p.isNotEmpty && !_sameUserId(p['_id'] ?? p['id'], currentId)) {
          return p;
        }
      }
    }
  ...
  Widget _buildConversationTile(ConversationModel c) {
    return InkWell(
      onTap: () async {
        await Navigator.pushNamed(context, '/messages/${c.id}');
