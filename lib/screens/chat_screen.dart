...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void dispose() {
    ...
    _scrollCtrl.dispose();
    if (_socket != null) {
      _socket!.emit('mark_messages_read', {'chatId': widget.chatId});
      _socket!.emit('leave_chat', widget.chatId);
      _socket!.disconnect();
      _socket!.dispose();
    }
    super.dispose();
  ...
  void _registerSocketListeners() {
    ...
    _socket!.on('new_message', (data) {
      ...
      if (!alreadyExists) {
        ...
        final myId = _currentUser?['_id']?.toString() ?? _currentUser?['id']?.toString();
